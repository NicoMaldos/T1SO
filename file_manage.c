...
static int rev = 1, fd1[2], fd2[2], fd3[3], fd4[2];
...
void turno(char *folder){
	...
	char cartdes[50] = "ult_descarte/", mano[100][50], cartjug[50] = "", input_str[100] = "";
	int cont = 0, cont2 = 0, optint, optint2, seguir = 0, seguir1 = 0;
	...
	while(seguir1 == 0){
		...
		if(cont2 == 0){
			...
			if(strcmp(cartjug, "+2") == 0){
				printf("El siguiente jugador debe sacar 2 cartas! ¡Pierde turno!\n");
				for(i=0; i<2; i++){
					//getCard(sig);
				}
				callFork(folder, "subsig", rev);
			}
			else if(strcmp(cartjug, "+4") == 0){
				printf("El siguiente jugador debe sacar 4 cartas! ¡Pierde turno!\n");
				for(i=0; i<4; i++){
					//getCard(sig);
				}
				callFork(folder, "subsig", rev);
				...
				rev = rev * -1;
				callFork(folder, "ant", rev);
			}
			else if(strcmp(cartjug, "cambio") == 0){
				callFork(folder, "sig", rev);
			}
			else{
				callFork(folder, "sig", rev);
			} 
...
void turno1(){
	DIR *dir;
	struct dirent *sd; 
	char  *descarte = "", optstr[2] = "", cartdes[50] = "", aux[50] = "", input_str[100] = "";
	pid_t p1, p2, p3;
	int i, optint = 0;
	...
	if(p1 == 0){
		printf("Hijo 1 in the house, %d.\n", getpid());
		read(fd2[0], input_str, 100);
		turno("jugador 2");
	}
	else if(p2 == 0){
		printf("Hijo 2 in the house, %d.\n", getpid());
		read(fd3[0], input_str, 100);
		turno("jugador 3");
	}
	else if(p3 == 0){
		printf("Hijo 3 in the house, %d.\n", getpid());
		read(fd4[0], input_str, 100);
		turno("jugador 4");
	}
...
void callFork(char *folder, char *puesto,int rev){
	...
	if(rev == 1){
				if(strcmp(folder, "jugador 1") == 0){
					if(strcmp(puesto, "ant") == 0){
						write(fd4[1], input_str, 100); 
					}
					else if(strcmp(puesto, "sig") == 0){
						write(fd2[1], input_str, 100); 
					}
					else{
						write(fd3[1], input_str, 100); 
